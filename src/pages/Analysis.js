...
import { useParams, useNavigate } from "react-router-dom";
// import the local PDF viewer you created
...
export default function Analysis() {
  ...
  useEffect(() => {
    function update() {
      ...
      const target = Math.min(Math.max(w - 80, 600), 1100);
      setPageWidth(target);
    }
    update();
  ...
  // Helper: get contract list for user and find s3Key for this contractId
  // Updated FIXED function for Analysis.js
const findContractS3Key = async (userId, cId) => {
  try {
    // 1. Get the token
    const idToken = sessionStorage.getItem("idToken"); 

    const resp = await fetch(`${API_URL}/contracts?userId=${encodeURIComponent(userId)}`, {
      method: "GET",
      headers: { 
        "Content-Type": "application/json",
    ...
    if (!resp.ok) {
      console.warn("findContractS3Key: failed to fetch contracts list", resp.status);
      return null;
    }
    const list = await resp.json();
    if (!Array.isArray(list)) return null;
    const found = list.find((it) => it.contractId === cId || it.contractId === cId.replace(/_/g, "_"));
    return found?.s3Key || found?.objectKey || null;
  } catch (e) {
    console.error("findContractS3Key error", e);
    return null;
  }
};

  // Helper: ask backend to presign GET url for objectKey
  const getSignedPdfUrl = async (objectKey) => {
    ...
    try {
      const resp = await fetch(`${API_URL}/contracts`, {
        ...
        body: JSON.stringify({ action: "get", objectKey, userId }),
      });
      const text = await resp.text();
      if (!resp.ok) {
        console.warn("getSignedPdfUrl failed", resp.status, text);
        return null;
      }
      const parsed = (() => { try { return JSON.parse(text); } catch { return {}; } })();
      const body = typeof parsed.body === "string" ? JSON.parse(parsed.body) : parsed;
      return body.getUrl || body.url || body.signedUrl || body.downloadUrl || body.viewUrl || null;
    } catch (e) {
      console.error("getSignedPdfUrl error", e);
      return null;
    }
  };

  // When we have analysis data, try to locate PDF and presign URL
  useEffect(() => {
    let mounted = true;
    (async () => {
      if (!data) {
      ...
      // Priority: if analysis contains a direct s3 key (maybe under sourceDocumentKey or s3Key), prefer that
      const directKeys = [
        data.s3Key,
        data.s3Key?.S,
        data.sourceDocumentKey,
        data.sourceDocumentKey?.S,
        data.sourceTextKey,
        data.sourceTextKey?.S,
        data.textKey,
        data.textKey?.S,
      ];

      let objectKey = directKeys.find(k => typeof k === "string" && k.toLowerCase().endsWith(".pdf"));
      if (!objectKey) {
        // fallback: fetch the Contracts table for the user to find s3Key
        const userId = getUserId();
        if (userId) {
          objectKey = await findContractS3Key(userId, contractId);
        }
      }

      if (!objectKey) {
        const maybeTxt = data.sourceTextKey || data.sourceTextKey?.S;
        if (typeof maybeTxt === "string" && maybeTxt.startsWith("uploads/") && maybeTxt.endsWith(".txt")) {
          const parts = maybeTxt.split("/");
          const userId = parts[1];
          const candidate = await findContractS3Key(userId, contractId);
          if (candidate) objectKey = candidate;
        }
      }

      if (objectKey) {
        const signed = await getSignedPdfUrl(objectKey);
        if (mounted) setPdfUrl(signed);
      } else {
  ...
  // Build highlights from clauses + red flags
  // In Analysis.js
...
//   // Helper: Select the best short text to search for
//   const getSearchText = (item, type) => {
//     // 1. If we have an exact quote from the backend, use it.
//     if (item.quote && item.quote.length > 3 && item.quote.length < 100) return item.quote;
    
//     // 2. If it's a Clause, search for its Name (Title) which usually exists as a header
//     if (type === 'clause' && item.name && item.name.length > 3) return item.name;

//     // 3. If it's a short Red Flag (e.g. "Missing Date"), use it. 
//     // If it's a long sentence (summary), ignore it because it won't be found in the PDF.
//     if (typeof item === 'string' && item.length < 50) return item;
    
//     return ""; 
//   };

//   const clauseHighlights = (data.Clauses || []).map((c, i) => ({
//     id: `clause-${i}`,
//     text: getSearchText(c, 'clause'), 
//     color: (c.risk || "").toLowerCase() === "high" ? "rgba(255, 90, 90, 0.4)" // Red
//          : (c.risk || "").toLowerCase() === "medium" ? "rgba(255, 200, 0, 0.4)" // Orange
//          : "rgba(100, 200, 100, 0.4)", // Green
//   }));

//   const rfHighlights = (data.RedFlags || []).map((r, i) => ({
//     id: `rf-${i}`,
//     text: getSearchText(r, 'redflag'),
//     color: "rgba(255, 0, 0, 0.3)",
//   }));

//   // Only return highlights that have text and aren't too long (likely summaries)
//   return [...clauseHighlights, ...rfHighlights].filter(h => h.text && h.text.length > 2);
// };
const buildHighlights = () => {
  if (!data) return [];
  ...
  // Helper: Extract searchable text from various formats
  const extractSearchText = (item, type) => {
    // For clauses, prioritize the name/title
    if (type === 'clause') {
      if (item.name && item.name.length >= 4 && item.name.length <= 60) {
        return item.name;
      }
      
      // Fallback to first sentence of summary if name is missing
      if (item.summary) {
    ...
    // For red flags - only use if it's short and specific
    if (type === 'redflag' && typeof item === 'string') {
      // Remove common prefixes that won't be in the PDF
      const cleaned = item
        .replace(/^(missing|no|lack of|unclear|vague|ambiguous):\s*/i, '')
        .trim();
  ...
  (data.Clauses || []).forEach((clause, i) => {
    const searchText = extractSearchText(clause, 'clause');
    if (searchText) {
      const riskLevel = (clause.risk || '').toLowerCase();
      highlights.push({
        id: `clause-${i}`,
        text: searchText,
        color: riskLevel === 'high' ? 'rgba(239, 68, 68, 0.35)' :
               riskLevel === 'medium' ? 'rgba(251, 191, 36, 0.35)' :
               'rgba(34, 197, 94, 0.35)'
      });
    }
  });
  ...
  (data.RedFlags || []).forEach((flag, i) => {
    const searchText = extractSearchText(flag, 'redflag');
    if (searchText) {
      highlights.push({
        id: `redflag-${i}`,
        text: searchText,
  ...
  console.log('Built highlights:', highlights);
  return highlights;
};
...
// ... rest of component
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-regal-offwhite to-regal-beige/30 px-6 py-8" ref={containerRef}>
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-regal-burgundy">Contract Analysis</h1>
          <div className="flex items-center gap-3">
          ...
          <>
                ...
                <div className="bg-white rounded-xl border p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          const textKey = data.sourceTextKey || data.sourceTextKey?.S || data.textKey || data.textKey?.S;
                          if (!textKey) return alert("No extracted text available");
                          alert("To download, implement a /file presign endpoint or copy the S3 key: " + textKey);
                        }}
                        className="px-3 py-2 rounded bg-white border"
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-regal-burgundy">Document Preview</h3>
                <div className="text-sm text-regal-black/60">Highlights are driven from extracted clauses & red flags</div>
              </div>

              <div className="bg-white rounded-xl border p-4">
                {pdfUrl ? (
                  <AnalysisPDF url={pdfUrl} highlights={buildHighlights()} pageWidth={pageWidth} />
                ) : (
                  <div className="text-sm text-regal-black/70">
                    PDF preview not available. (If you expected a preview, ensure the contract's S3 key is present in your Contracts table.)
                  </div>
                )}
              </div>
            </div>
